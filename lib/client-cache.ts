...
export function readMarketplaceCache(maxAgeMs=10*60*1000):any[]{if(typeof window==="undefined")return[];try{const raw=localStorage.getItem(MARKETPLACE_KEY);if(!raw)return[];const v=JSON.parse(raw);return Array.isArray(v?.products)&&Date.now()-Number(v?.ts||0)<=maxAgeMs?v.products:[]}catch{return[]}}
