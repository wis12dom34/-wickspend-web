...
export default function LiveAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const [next, market, firstUsers] = await Promise.all([
          api.admin.stats(token) as Promise<AdminStats>,
          api.admin.marketplaceOrders(token).catch(() => ({ orders: [] })) as Promise<any>,
          api.admin.users(token,{limit:100,page:1}).catch(() => ({ users: [], total: 0 })) as Promise<any>,
        ]);
        const totalUsers = Number(firstUsers?.total || 0);
        const pages = Math.max(1, Math.ceil(totalUsers / 100));
        const extraPages = pages > 1 ? await Promise.all(Array.from({length:pages-1},(_,i)=>api.admin.users(token,{limit:100,page:i+2}).catch(() => ({users:[]})))) : [];
        const adminUsers = [firstUsers,...extraPages].flatMap((r:any)=>Array.isArray(r?.users)?r.users:[]);
        const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0,0,0,0);
        const activeThisMonth = adminUsers.filter((u:any)=>u?.last_seen_at && new Date(u.last_seen_at).getTime() >= monthStart.getTime()).length;
        if (alive) {
          setStats(next);
          setMonthlyUsers(activeThisMonth);
          const orders = Array.isArray(market) ? market : (market?.orders || market?.data || []);
          setUserActivity(Array.isArray(orders) ? orders.slice(0, 20) : []);
        }
  ...
  const statisticItems = [
    ...
    { icon: 'users' as const, label: 'Monthly Users', value: monthlyUsers === null ? '—' : count(monthlyUsers), note: 'Active this month', href: routes.users, trend: [] },
  ...
  const recent = [...userActivity].sort((a,b) => new Date(b.created_at||b.date||0).getTime() - new Date(a.created_at||a.date||0).getTime()).slice(0, 8);
  ...
  return <main className={`${styles.page} wick-dashboard-page`} data-node-id="497:3280">
    ...
    <section className="ws-card ws-recent">
      <div className="ws-section-head"><div><strong>Recent User Activity</strong><span>See who bought what · {generatedLabel}</span></div><Link href="/admin/marketplace/orders">View all</Link></div>
      {recent.length ? <div className="ws-recent-list">{recent.map((tx, i) => { const when=tx.created_at||tx.date; const amount=tx.price_ngn??tx.amount_ngn; return <Link href="/admin/marketplace/orders" className="ws-transaction-row" key={tx.id||tx.reference||`${when}-${i}`}><div className="ws-activity-copy"><strong>{activityName(tx)}</strong><span className="ws-activity-action">{activityAction(tx)}</span><small>{when ? new Date(when).toLocaleString('en-NG',{dateStyle:'medium',timeStyle:'short'}) : 'Recent'} · {String(tx.status||'completed').replaceAll('_',' ')}</small></div><b>{amount!==undefined ? money(amount) : 'View'}</b></Link>})}</div> : <div className="ws-empty">No recent user purchases are available yet.</div>}
    </section>
