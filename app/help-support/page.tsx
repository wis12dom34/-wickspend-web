...
const categories=["Numbers","OTP","Rentals","Marketplace","Wallet","Payments","Account"] as const;
const faqs=[
...
export default function HelpSupport(){
  const[query,setQuery]=useState(""),[category,setCategory]=useState<string>(""),[open,setOpen]=useState<number>(4);
  const normalized=query.trim().toLowerCase();
  const visible=useMemo(()=>faqs.map((faq,index)=>({faq,index})).filter(({faq})=>!normalized||`${faq[0]} ${faq[1]}`.toLowerCase().includes(normalized)),[normalized]);
  return <PageShell title="Help & Support" subtitle="Find answers or send us the details we need to help." back="/settings">
    <section style={{marginTop:8}}>
      ...
      <label style={{height:48,borderRadius:22,background:"#f9f9fa",border:"1px solid rgba(0,0,0,.07)",display:"flex",alignItems:"center",padding:"0 20px"}}>
        <input value={query} onChange={event=>{setQuery(event.target.value);setCategory("")}} placeholder="Search help articles and FAQs" aria-label="Search help articles and FAQs" style={{width:"100%",border:0,outline:0,background:"transparent",fontSize:10,color:"#111"}}/>
      </label>
    </section>

    <section style={{marginTop:26}}>
      <h2 style={{fontSize:13,margin:"0 0 12px"}}>Categories</h2>
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>{categories.map(item=>{
        const selected=category===item;
        return <button key={item} type="button" aria-pressed={selected} onClick={()=>{setCategory(selected?"":item);setQuery(selected?"":item)}} style={{height:38,borderRadius:19,border:"1px solid rgba(0,0,0,.07)",background:selected?"#000":"rgba(255,255,255,.95)",color:selected?"#fff":"#000",padding:"0 18px",fontSize:9,fontWeight:600}}>{item}</button>
      })}</div>
    </section>
    ...
    <section style={{marginTop:18}}>
      ...
      <div style={{display:"grid",gap:10}}>{visible.length?visible.map(({faq:[question,answer],index})=>{
        ...
        return <article key={question} style={{border:"1px solid rgba(0,0,0,.07)",borderRadius:20,background:expanded?"#f9f9fa":"rgba(255,255,255,.95)",boxShadow:"0 6px 18px rgba(0,0,0,.08)",overflow:"hidden"}}>
          <button type="button" aria-expanded={expanded} onClick={()=>setOpen(expanded?-1:index)} style={{width:"100%",minHeight:54,border:0,background:"transparent",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"0 15px",textAlign:"left",fontSize:10,fontWeight:600}}><span>{question}</span><span aria-hidden="true">{expanded?"⌃":"⌄"}</span></button>
