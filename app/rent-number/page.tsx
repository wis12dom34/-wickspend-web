...
import clean from "./rent-number-clean.module.css";
...
export default function RentNumber(){
 ...
 return <main className={clean.page}>
  ...
  <div className={clean.selectors}>
    <button type="button" className={clean.selector} onClick={()=>router.push("/buy-number?country=USA")}>
      <small>1. Country</small><div className={clean.selectorValue}><span className={clean.flag}>🇺🇸</span><strong>United States</strong><span className={clean.chevron}>⌄</span></div>
    </button>
    <div className={clean.selector}>
      <small>2. Service</small><div className={clean.serviceSelectWrap}><span className={clean.serviceLogo}>●</span><select aria-label="Rental service" value={serviceCode} disabled={!catalog.length||busy} onChange={e=>setServiceCode(e.target.value)}>{catalog.length?catalog.map((x:any)=>{const c=serviceId(x);return <option value={c} key={c}>{x.name||x.service_name||x.title||c}</option>}):<option value="">No services</option>}</select><span className={clean.chevron}>⌄</span></div>
    </div>
  </div>

  <section className={clean.stats} aria-label="Service information">
    <div className={clean.stat}><span className={clean.statIcon}>◇</span><div><b>99.3%</b><small>Success rate</small></div></div>
    <div className={clean.stat}><span className={clean.statIcon}>ϟ</span><div><b>Instant</b><small>Delivery</small></div></div>
    <div className={clean.stat}><span className={clean.statIcon}>◷</span><div><b>20 Min</b><small>Validity</small></div></div>
    <div className={clean.stat}><span className={clean.statIcon}>↻</span><div><b>Refund</b><small>If failed</small></div></div>
  </section>

  <section className={clean.tabs} aria-label="Number type">
    <button className={clean.tab} onClick={()=>router.push("/buy-number")}>One-Time</button>
    <button className={`${clean.tab} ${clean.activeTab}`} aria-current="page">Rental</button>
    <button className={clean.tab} onClick={()=>router.push("/numbers/history")}>Favorites</button>
  </section>
  <p className={clean.hint}>Rent a number for a longer period. Receive OTPs anytime.</p>

  <form onSubmit={createRental} className={clean.listCard}>
    <div className={clean.listHead}><h2>Available Rental Numbers</h2><button type="button" className={clean.refresh} disabled={refreshing} onClick={refreshCatalog}>{refreshing?"Refreshing…":"↻ Refresh"}</button></div>
    <div className={clean.cols}><span>Rental Period</span><span>Price</span><span>Status</span><span>Action</span></div>
    <div className={clean.rows}>
      {durations.map(([v,label])=><div className={`${clean.rentalRow} ${duration===v?clean.selectedRow:""}`} key={v}>
        <button type="button" className={clean.duration} onClick={()=>setDuration(v)}><span className={clean.calendarIcon}>□</span><strong>{label}</strong><span>⌄</span></button>
        <span className={clean.price}>{priceOf(selected)}</span>
        <span className={clean.available}><i/>Available</span>
        <button type="submit" className={clean.rentBtn} disabled={busy||!serviceCode} onClick={()=>setDuration(v)}>{busy&&duration===v?"Renting…":"Rent"}</button>
      </div>)}
    </div>
    <button type="button" className={clean.more} onClick={refreshCatalog}>View more numbers <span>⌄</span></button>
  </form>
  {message&&<p className={clean.message} role="status">{message}</p>}
  <BottomNav/>
 </main>
}
