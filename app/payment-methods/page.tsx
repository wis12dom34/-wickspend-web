...
const supported=new Set(["NGN","GHS","KES","ZAR","XAF","XOF","USDT","USDC"]);

export default function PaymentMethods(){
  const[currency,setCurrency]=useState("NGN");
  useEffect(()=>{const saved=localStorage.getItem("wickspend_display_currency");if(saved&&supported.has(saved))setCurrency(saved)},[]);
  return <PageShell title="Payment Methods" subtitle="Choose how you fund your wallet" back="/profile">
    <section className="walletHero" style={{height:110,background:"#050505",color:"#fff"}}>
      ...
      <small style={{display:"block",marginTop:18,color:"#fff",fontWeight:600}}>Preferred currency&nbsp; • &nbsp;{currency}</small>
    </section>
    <section className="profileGroup"><p>Available methods</p><div className="profileRows">
      ...
      <div className="profileRow" aria-disabled="true" title="Mobile money is not exposed by the current funding API"><span aria-hidden="true">◉</span><b>Mobile money<small style={{display:"block",fontWeight:400,color:"#6e6e73",marginTop:4}}>Availability depends on country</small></b><i aria-hidden="true">›</i></div>
    </div></section>
