...
export default function BuyNumberClient({ initialCountry = "19", initialService = "telegram", initialPremium = false, seoTitle = "Buy Virtual Numbers Online", seoIntro = "Buy virtual numbers for supported services and countries on WickSpend. Choose a country and service, view current availability and live pricing, then complete your purchase securely from your WickSpend account." }: { initialCountry?: string; initialService?: string; initialPremium?: boolean; seoTitle?: string; seoIntro?: string }) {
  const router = useRouter();
  const premium = initialPremium;
  const [countries, setCountries] = useState<CountryOption[]>(fallbackCountries);
  const [serviceOptions, setServiceOptions] = useState<ServiceOption[]>([]);
  const [country, setCountry] = useState(initialCountry || "19");
  const [service, setService] = useState(initialService || "telegram");
  ...
  useEffect(() => {
    let cancelled = false;
    const requestedCountry = initialCountry;
  ...
  useEffect(() => {
    ...
    api.numbers.services("", country).then((data: any) => {
      ...
      if (!next.length) return;
      setServiceOptions(next);
      const requested = String(initialService || "").toLowerCase();
      setService((current) => {
  ...
  return (
    <PageShell title={seoTitle} subtitle={premium ? "Choose a Premium USA service" : "Choose a country and service"}>
      <section className="seoIntroCard" aria-label="About buying virtual numbers"><p>{seoIntro}</p></section>
      ...
      <section className="seoContent" aria-label="Virtual number buying guide">
        <h2>How buying a virtual number works</h2>
        <p>Select a supported country and service, then use <strong>View Prices</strong> to check current provider offers. Pricing, stock and availability stay live and are confirmed during purchase.</p>
        <h2>How OTP and SMS delivery works</h2>
        <p>After a successful purchase, WickSpend opens the OTP screen for that number. Incoming messages appear there when the provider receives them. Delivery timing depends on the selected service and provider.</p>
        <h2>When a number is unavailable</h2>
        <p>If a selected offer is no longer available, WickSpend does not treat the purchase as successful. Follow the on-screen status, refresh the live offers and choose another available option when needed.</p>
        <h2>Important availability information</h2>
        <p>Countries, services, stock and prices can change. The live selector and checkout state are the source of truth; this page does not hard-code changing inventory or prices.</p>
        <h2>Buy Number FAQ</h2>
        <details><summary>Are prices fixed?</summary><p>No. WickSpend shows current live pricing for the country, service and provider offer you select.</p></details>
        <details><summary>Can I keep a number for longer-term access?</summary><p>For supported longer-term use cases, visit <a href="/rent-number">Rent Number</a> and check the rental periods currently available.</p></details>
        <details><summary>Where can I learn how to use WickSpend?</summary><p>See the <a href="/tutorials">WickSpend Tutorials</a> for published walkthroughs, including number and OTP guides when available.</p></details>
        <nav className="seoRelatedLinks" aria-label="Related WickSpend services"><a href="/rent-number">Rent a virtual number</a><a href="/tutorials">View tutorials</a><a href="/marketplace">Browse Marketplace</a></nav>
      </section>
    </PageShell>
  );
}