...
export default function TutorialsPage(){
  ...
  async function copyLink(t:Tutorial){const url=`${window.location.origin}/tutorials/${t.id}`;try{await navigator.clipboard.writeText(url);setToast("Tutorial link copied")}catch{setToast("Couldn’t copy the link")}setMenuId(null)}
  async function share(t:Tutorial){const url=`${window.location.origin}/tutorials/${t.id}`;try{if(navigator.share)await navigator.share({title:t.title,text:t.description||undefined,url});else await copyLink(t)}catch{}finally{setMenuId(null)}}
