...
export default function Profile(){
 ...
 useEffect(()=>{let active=true;const token=getSessionToken();if(!token){setSignedIn(false);setMessage("You are not signed in.");return}setSignedIn(true);api.auth.session(token).then((data:any)=>{if(!active)return;setUser(data?.user||data?.session?.user||data?.data?.user||data?.data||data);setMessage("")}).catch(()=>{if(!active)return;clearSessionToken();setSignedIn(false);setUser(null);setMessage("Your session has expired. Please sign in again.")});return()=>{active=false}},[]);
