...
import {api,wickspendApi,newRequestKey} from "@/lib/api";
import {getSessionToken} from "@/lib/session";
import {readMarketplaceCache,writeMarketplaceCache} from "@/lib/client-cache";
import styles from "./marketplace.module.css";

const idOf=(p:any)=>String(p?.product_id??p?.id??p?.code??"");
const isManual=(p:any)=>idOf(p).startsWith("manual:");
const nameOf=(p:any)=>String(p?.name??p?.title??p?.product_name??"Product");
const stockOf=(p:any)=>{const raw=p?.remaining_stock??p?.available_inventory??p?.inventory_available??p?.stock??p?.quantity_available??p?.available_stock??p?.stock_quantity??p?.available??null;const n=Number(raw);return raw!==null&&raw!==undefined&&raw!==""&&Number.isFinite(n)?n:null};
...
export default function MarketplaceClient({initialProducts=[]}:{initialProducts?:any[]}){
 const initialVisible=initialProducts.filter(isInStock);
 const[products,setProducts]=useState<any[]>(initialVisible),[selected,setSelected]=useState<any|null>(null),[query,setQuery]=useState(""),[category,setCategory]=useState("All"),[loading,setLoading]=useState(initialVisible.length===0),[busy,setBusy]=useState(false),[detailRefreshing,setDetailRefreshing]=useState(false),[liveVerified,setLiveVerified]=useState(false),[message,setMessage]=useState(""),[confirm,setConfirm]=useState(false),[result,setResult]=useState<any|null>(null),[renderLimit,setRenderLimit]=useState(30),[refreshKey,setRefreshKey]=useState(0);
 useEffect(()=>{let last=Date.now();const refresh=()=>{const now=Date.now();if(now-last<1500)return;last=now;setRefreshKey(v=>v+1)};const visible=()=>{if(document.visibilityState==="visible")refresh()};window.addEventListener("focus",refresh);document.addEventListener("visibilitychange",visible);return()=>{window.removeEventListener("focus",refresh);document.removeEventListener("visibilitychange",visible)}},[]);
 useEffect(()=>{let cancelled=false;
  const all:any[]=[...initialVisible];const seen=new Set<string>(all.map(idOf));
  const merge=(payload:any)=>{let added=0;for(const p of listOf(payload).filter((x:any)=>idOf(x))){const id=idOf(p);if(!seen.has(id)){seen.add(id);all.push(p);added++}}return added};
  const publish=()=>{if(cancelled||!all.length)return;const visible=all.filter(isInStock);setProducts([...visible]);setLoading(false);setMessage("");writeMarketplaceCache(visible)};
  const cached=readMarketplaceCache();if(cached.length){setProducts(cached.filter(isInStock));setLoading(false)}
  const manualTask=(async()=>{try{const manual:any=await wickspendApi("wickspend/backend/marketplace/manual-products?page=1&limit=50",{timeoutMs:8000});if(cancelled)return;if(manual){merge(manual);publish()}}catch{}})();
  const providerTask=(async()=>{
   try{
    const first:any=await api.marketplace.products({page:1,limit:50});if(cancelled)return;merge(first);publish();
    const total=Number(first?.total??first?.pagination?.total??first?.data?.total);
    if(hasMoreOf(first)&&Number.isFinite(total)&&total>50){const pages=Math.min(20,Math.ceil(total/50));const rest=await Promise.all(Array.from({length:pages-1},(_,i)=>api.marketplace.products({page:i+2,limit:50}).catch(()=>null)));if(cancelled)return;for(const d of rest)if(d)merge(d);publish()}
    else if(hasMoreOf(first)){for(let page=2;page<=20;page++){const d:any=await api.marketplace.products({page,limit:50});if(cancelled)return;const batch=listOf(d);const added=merge(d);if(!hasMoreOf(d)||batch.length===0||added===0)break}publish()}
   }catch(e){if(!cancelled&&!cached.length&&all.length===0){setProducts([]);setMessage(e instanceof Error?e.message:"Marketplace unavailable right now.");setLoading(false)}}
  })();
  Promise.allSettled([manualTask,providerTask]).then(()=>{if(!cancelled&&all.length===0&&!cached.length){setLoading(false)}});
 ...
 async function getLive(p:any,timeoutMs=5000){const id=idOf(p);return isManual(p)?wickspendApi(`wickspend/backend/marketplace/manual-product?product_id=${encodeURIComponent(id)}`,{timeoutMs}):wickspendApi(`wickspend/backend/marketplace/product?product_id=${encodeURIComponent(id)}`,{timeoutMs})}
 async function open(p:any){setSelected(p);setLiveVerified(priceOf(p)!==null&&isInStock(p));setDetailRefreshing(false);setMessage("");try{const d:any=await getLive(p,4000);const live=d?.product??d?.data?.product??d?.data??d;if(idOf(live)===idOf(p)){setSelected(live);setLiveVerified(priceOf(live)!==null&&isInStock(live))}}catch{}}
 async function purchase(){if(!selected||busy||detailRefreshing||!liveVerified)return;const token=getSessionToken();if(!token){setConfirm(false);setMessage("Please sign in before purchasing.");return}setBusy(true);setMessage("");try{const r:any=isManual(selected)?await wickspendApi("wickspend/backend/marketplace/manual-purchase",{method:"POST",token,body:JSON.stringify({product_id:idOf(selected),request_key:newRequestKey("market-manual")})}):await api.marketplace.reserve(token,{product_id:idOf(selected),quantity:1});if(isManual(selected))api.wallet.invalidate(token);setResult(r);setConfirm(false)}catch(e){setConfirm(false);setMessage(e instanceof Error?e.message:"Unable to submit this Marketplace order.")}finally{setBusy(false)}}
 const resultStatus=statusOf(result),resultRef=refOf(result),pending=!!result&&(result?.provider_pending===true||/processing|pending|queued|confirming/.test(resultStatus)),failed=!!result&&/failed|rejected|cancelled|canceled|error/.test(resultStatus),confirmed=!!result&&!pending&&!failed&&(!!resultRef||/success|successful|completed|fulfilled|delivered|active/.test(resultStatus));
 if(result){const manual=isManual(selected);return <main className={`${styles.screen} ${styles.stateScreen}`}><header className={styles.topHeader}><div><h1>Marketplace</h1><p>Order status</p></div><Link href="/orders" prefetch={false} className={styles.ordersButton}>Orders</Link></header><section className={styles.stockCard}><span>{pending?"PROCESSING":failed?"NOT COMPLETED":confirmed?"ORDER SUBMITTED":"STATUS"}</span><h2>{pending?(manual?"Your order is awaiting delivery":"We’re confirming your order"):failed?"Marketplace order not completed":confirmed?"Order submitted":"Order status unavailable"}</h2><b>{nameOf(selected)}</b><strong>{money(priceOf(selected))}</strong></section><section className={styles.detailCopy}><h3>{pending?(manual?"Pending delivery":"Provider confirmation pending"):failed?"Order not completed":"Track this order"}</h3><p>{pending?(manual?"Your payment and order are recorded. Delivery will appear in Orders when it is ready.":"Your request was accepted and is still being confirmed. Do not place a duplicate order."):failed?"The order did not complete. Check Orders and your wallet before trying again.":confirmed?`Reference${resultRef?` • ${resultRef}`:" available in Orders"}. Your delivery is protected and available from your order.`:"The backend response did not contain enough information to confirm completion."}</p></section><Link href={resultRef?`/orders?reference=${encodeURIComponent(resultRef)}`:"/orders"} className={styles.notifyButton}>View Orders</Link><button className={styles.notifyButton} type="button" onClick={()=>{setResult(null);setSelected(null)}}>Back to Marketplace</button><BottomNav/></main>}
 if(selected){const stock=stockOf(selected),price=priceOf(selected);return <main className={`${styles.screen} ${styles.detailScreen}`}><header className={styles.topHeader}><div><h1>Marketplace</h1><p>Product details</p></div><Link href="/orders" prefetch={false} className={styles.ordersButton}>Orders</Link></header><button className={styles.backLink} type="button" onClick={()=>{setSelected(null);setLiveVerified(false);setDetailRefreshing(false)}}>‹ Back to Marketplace</button>{detailRefreshing&&<div className={styles.detailLoader}><i/><span>Refreshing live price & stock…</span></div>}<section className={styles.heroCard}><span>LIVE PRODUCT</span><h2>{nameOf(selected)}</h2><p>{categoryLabel(selected)}</p><div><strong>{money(price)}</strong><b>{stock===null?"In stock":`${stock} in stock`}</b></div></section><section className={styles.detailCopy}><h3>About this product</h3><p>{selected?.description||selected?.short_description||"Marketplace product details supplied by the WickSpend backend."}</p></section><section className={styles.detailCopy}><h3>{selected?.delivery_label||"Live pricing & availability"}</h3><p>{isManual(selected)?"Delivery is released only after a successful wallet purchase and is visible only from the purchasing account.":"Price and stock are loaded from the verified Marketplace backend before purchase. Final order status is confirmed by the provider workflow."}</p></section><div className={styles.purchaseRow}><div className={styles.qtyBlock}><label>Quantity</label><div><button disabled>−</button><strong>1</strong><button disabled>＋</button></div></div><button className={styles.saveButton} type="button" onClick={()=>{setSelected(null);setLiveVerified(false);setDetailRefreshing(false)}}>Cancel</button><button className={styles.buyNow} type="button" disabled={busy||detailRefreshing||!liveVerified||price===null||!isInStock(selected)} onClick={()=>setConfirm(true)}>{detailRefreshing?"Refreshing…":busy?"Loading…":liveVerified?"Buy now":"Verify live data"}</button></div>{message&&<p className={styles.centerNotice}>{message}</p>}{confirm&&<div className={styles.confirmBackdrop}><section className={styles.confirmSheet}><h2>Confirm purchase</h2><div className={styles.confirmRows}><div><span>Product</span><strong>{nameOf(selected)}</strong></div><div><span>Quantity</span><strong>1</strong></div><div><span>Total</span><strong>{money(price)}</strong></div><div><span>Stock</span><strong>{stock===null?"Available":`${stock} in stock`}</strong></div></div><button className={styles.confirmButton} type="button" disabled={busy} onClick={purchase}>{busy?"Processing…":"Confirm & Pay"}</button><button className={styles.cancelButton} type="button" disabled={busy} onClick={()=>setConfirm(false)}>Cancel</button></section></div>}<BottomNav/></main>}
 return <main className={styles.screen}><header className={styles.topHeader}><div><h1>Marketplace</h1><p>Premium digital products, one clean checkout.</p></div><Link href="/orders" prefetch={false} className={styles.ordersButton}>Orders</Link></header><label className={styles.searchBox}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search live Marketplace products" aria-label="Search Marketplace"/></label>{!loading&&!message&&categories.length>1&&<div className={styles.categoryBar} aria-label="Marketplace categories">{categories.map(c=><button key={c} type="button" className={category===c?styles.categoryActive:""} onClick={()=>setCategory(c)}>{c}</button>)}</div>}{loading?<section className={styles.productSection}><div className={styles.sectionTitle}><h2>Loading Marketplace</h2><p>Checking the full live catalog, prices and stock.</p></div><div className={styles.productGrid}>{Array.from({length:6}).map((_,i)=><SkeletonCard key={i}/>)}</div></section>:message?<div className={styles.emptyState}><strong>Marketplace unavailable</strong><p>{message}</p></div>:shown.length?(category==="All"?<>{groupedShown.map(([group,items])=><section className={styles.productSection} key={group}><div className={styles.sectionTitle}><h2>{group}</h2><p>{items.length} in-stock product{items.length===1?"":"s"}</p></div><div className={styles.productGrid}>{items.map(p=><ProductCard key={idOf(p)} product={p} busy={busy} onView={open}/>)}</div></section>)}</>:<section className={styles.productSection}><div className={styles.sectionTitle}><h2>{category}</h2><p>{shown.length} in-stock product{shown.length===1?"":"s"}</p></div><div className={styles.productGrid}>{renderedShown.map(p=><ProductCard key={idOf(p)} product={p} busy={busy} onView={open}/>)}</div></section>):<div className={styles.emptyState}><strong>No products found</strong><p>{query||category!=="All"?"Try another search or category.":"No in-stock Marketplace products are available right now."}</p></div>}{shown.length>renderLimit&&<button type="button" className={styles.loadMore} onClick={()=>setRenderLimit(v=>Math.min(v+30,shown.length))}>Load more products</button>}<BottomNav/></main>;
}
