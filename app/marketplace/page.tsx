...
export default function Marketplace(){
 const[products,setProducts]=useState<any[]>([]),[selected,setSelected]=useState<any|null>(null),[query,setQuery]=useState(""),[category,setCategory]=useState("All"),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[message,setMessage]=useState(""),[confirm,setConfirm]=useState(false),[result,setResult]=useState<any|null>(null);
 ...
 if(selected){const stock=stockOf(selected),price=priceOf(selected);return <main className={`${styles.screen} ${styles.detailScreen}`}><header className={styles.topHeader}><div><h1>Marketplace</h1><p>Product details</p></div><Link href="/orders" className={styles.ordersButton}>Orders</Link></header><button className={styles.backLink} type="button" onClick={()=>setSelected(null)}>‹ Back to Marketplace</button><section className={styles.heroCard}><span>LIVE PRODUCT</span><h2>{nameOf(selected)}</h2><p>{categoryLabel(selected)}</p><div><strong>{money(price)}</strong><b>{stock===null?"In stock":`${stock} in stock`}</b></div></section><section className={styles.detailCopy}><h3>About this product</h3><p>{selected?.description||"Live Marketplace product details supplied by the WickSpend backend."}</p></section><section className={styles.detailCopy}><h3>Live pricing & availability</h3><p>Price and stock are loaded from the verified Marketplace backend before purchase. Final order status is confirmed by the provider workflow.</p></section><div className={styles.purchaseRow}><div className={styles.qtyBlock}><label>Quantity</label><div><button disabled>−</button><strong>1</strong><button disabled>＋</button></div></div><button className={styles.saveButton} type="button" onClick={()=>setSelected(null)}>Cancel</button><button className={styles.buyNow} type="button" disabled={busy||price===null||!isInStock(selected)} onClick={()=>setConfirm(true)}>{busy?"Loading…":"Buy now"}</button></div>{message&&<p className={styles.centerNotice}>{message}</p>}{confirm&&<div className={styles.confirmBackdrop}><section className={styles.confirmSheet}><h2>Confirm purchase</h2><div className={styles.confirmRows}><div><span>Product</span><strong>{nameOf(selected)}</strong></div><div><span>Quantity</span><strong>1</strong></div><div><span>Total</span><strong>{money(price)}</strong></div><div><span>Stock</span><strong>{stock===null?"Available":`${stock} in stock`}</strong></div></div><button className={styles.confirmButton} type="button" disabled={busy} onClick={purchase}>{busy?"Processing…":"Confirm & Pay"}</button><button className={styles.cancelButton} type="button" disabled={busy} onClick={()=>setConfirm(false)}>Cancel</button></section></div>}<BottomNav/></main>}
