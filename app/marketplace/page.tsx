...
const categoryLabel=(p:any)=>{
 ...
 if(["facebook","instagram","tiktok","telegram","x","twitter","reddit","dating","social","social_accounts"].includes(c))return"Social Accounts";
 if(["email","gmail","hotmail","outlook","digital_accounts"].includes(c))return"Digital Accounts";
 if(["vpn_proxy","vpn","proxy","tools","digital tools","digital_tools","software"].includes(c))return"Tools";
...
export default function Marketplace(){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{setLoading(true);try{const all:any[]=[];const seen=new Set<string>();for(let page=1;page<=20;page++){const d:any=await api.marketplace.products({page,limit:50});if(cancelled)return;const batch=listOf(d).filter((p:any)=>idOf(p));let added=0;for(const p of batch){const id=idOf(p);if(!seen.has(id)){seen.add(id);all.push(p);added++}}if(!hasMoreOf(d)||batch.length===0||added===0)break}const manual:any=await wickspendApi("wickspend/backend/marketplace/manual-products?page=1&limit=50");for(const p of listOf(manual)){const id=idOf(p);if(id&&!seen.has(id)){seen.add(id);all.push(p)}}if(cancelled)return;setProducts(all.filter(isInStock));setMessage("")}catch(e){if(!cancelled){setProducts([]);setMessage(e instanceof Error?e.message:"Marketplace unavailable right now.")}}finally{if(!cancelled)setLoading(false)}})();return()=>{cancelled=true}},[]);
