...
const brandIconUrl=(p:any)=>{
 ...
 if(category==="x"||category==="twitter")return "https://cdn.simpleicons.org/x";
...
export default function Marketplace(){
 ...
 useEffect(()=>{let cancelled=false;const cacheKey="wickspend:marketplace:v6",cacheTtl=5*60*1000;
  ...
  (async()=>{const all:any[]=[];const seen=new Set<string>();const merge=(payload:any)=>{let added=0;for(const p of listOf(payload).filter((x:any)=>idOf(x))){const id=idOf(p);if(!seen.has(id)){seen.add(id);all.push(p);added++}}return added};
   try{
    ...
    if(hasMoreOf(first))for(let page=2;page<=20;page++){const d:any=await api.marketplace.products({page,limit:50});if(cancelled)return;const batch=listOf(d);const added=merge(d);if(added)publish(all);if(!hasMoreOf(d)||batch.length===0||added===0)break}
   }catch(e){if(!cancelled){if(all.length===0){setProducts([]);setMessage(e instanceof Error?e.message:"Marketplace unavailable right now.")}setLoading(false)}}
 ...
 const groupedShown=useMemo(()=>{const groups=new Map<string,any[]>();for(const p of shown){const key=categoryLabel(p);const list=groups.get(key)??[];list.push(p);groups.set(key,list)}return Array.from(groups.entries()).sort(([a],[b])=>a.localeCompare(b))},[shown]);
