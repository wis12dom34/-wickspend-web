...
export default function Marketplace(){
 ...
 async function purchase(){if(!selected||busy||detailRefreshing||!liveVerified)return;const token=getSessionToken();if(!token){setConfirm(false);setMessage("Please sign in before purchasing.");return}setBusy(true);setMessage("");try{const d:any=await getLive(selected);const live=d?.product??d?.data?.product??d?.data??d;if(idOf(live)!==idOf(selected)||priceOf(live)===null||!isInStock(live)){setSelected(live||selected);setLiveVerified(false);setConfirm(false);setMessage("Live price or stock changed. Please review the updated product details.");return}setSelected(live);const r:any=isManual(live)?await wickspendApi("wickspend/backend/marketplace/manual-purchase",{method:"POST",token,body:JSON.stringify({product_id:idOf(live),request_key:newRequestKey("market-manual")})}):await api.marketplace.reserve(token,{product_id:idOf(live),quantity:1});if(isManual(live))api.wallet.invalidate(token);setResult(r);setConfirm(false)}catch(e){setConfirm(false);setMessage(e instanceof Error?e.message:"Unable to submit this Marketplace order.")}finally{setBusy(false)}}
