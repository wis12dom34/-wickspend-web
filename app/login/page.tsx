...
export default function Login(){
  ...
  const actionSeq=useRef(0),mounted=useRef(true),magicStarted=useRef(false),otpRef=useRef<HTMLInputElement|null>(null);
  ...
  useEffect(()=>{if(magicStarted.current||typeof window==="undefined")return;const params=new URLSearchParams(window.location.hash.replace(/^#/,""));const mail=(params.get("email")||"").trim().toLowerCase(),magicCode=(params.get("code")||"").replace(/\D/g,"").slice(0,6);if(!validEmail(mail)||magicCode.length!==6)return;magicStarted.current=true;window.history.replaceState(null,"",window.location.pathname+window.location.search);const seq=++actionSeq.current;setEmail(mail);setCode(magicCode);setStep("emailVerify");setBusy(true);setMessage("Signing you in securely…");api.auth.emailVerify({email:mail,code:magicCode}).then((r:any)=>{const t=sessionTokenOf(r);if(!t)throw new Error("Unable to complete sign in right now. Please try again.");saveSessionToken(String(t));if(mounted.current&&seq===actionSeq.current)window.location.replace(safeNext())}).catch(error=>{clearSessionToken();if(mounted.current&&seq===actionSeq.current)setMessage(error instanceof Error?error.message:"Secure sign-in failed")}).finally(()=>{if(mounted.current&&seq===actionSeq.current)setBusy(false)})},[]);
  ...
  async function completePasswordAuth(e:FormEvent){
    ...
    try{
      ...
      if(error instanceof ApiError&&error.code?.toUpperCase()==="PASSWORD_NOT_SET"){
        try{await api.auth.emailStart({email:mail});clearOtp();setStep("emailVerify");setMessage("We sent a secure sign-in link and code to your email.")}
