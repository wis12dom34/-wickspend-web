...
export default function Login(){
  ...
  async function completePasswordAuth(e:FormEvent){
    ...
    try{
      ...
      if(error instanceof ApiError&&error.code?.toUpperCase()==="PASSWORD_NOT_SET"){
        setStep("telegram");
        setMessage("This account does not have a password yet. Continue with your Telegram-linked WickSpend account.");
  ...
  const lead=step==="email"?(mode==="login"?"Enter your email and password to continue to WickSpend.":"Create your WickSpend account with email and password."):step==="telegram"?"Sign in securely with your Telegram-linked WickSpend account.":`Enter the verification code sent for Telegram ID ${telegramId}.`;
