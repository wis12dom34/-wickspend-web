...
export default function TempMailInbox(){
  ...
  const load=useCallback(async(silent=false)=>{const token=getSessionToken();if(!token){setMessage("Please sign in to open this inbox.");return}if(!silent)setRefreshing(true);try{const d:any=await api.tempMail.status(token,reference);const next=payloadOf(d);setOrder(next);sessionStorage.setItem(`wickspend.tempMailOrder.${reference}`,JSON.stringify(next));setMessage(emailOf(next)?"":"Preparing your email address…")}catch(e){setMessage(e instanceof Error?e.message:"Unable to refresh inbox")}finally{if(!silent)setRefreshing(false)}},[reference]);
  ...
  const status=String(order?.status||"Active"),expired=/expired|complete|cancel|refunded|closed|failed/i.test(status);
  useEffect(()=>{if(expired)return;const interval=emailOf(order)?8000:2500;const id=window.setInterval(()=>load(true),interval);return()=>window.clearInterval(id)},[expired,load,order]);
