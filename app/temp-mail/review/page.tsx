...
export default function ReviewMail(){
  ...
  async function create(){if(busy||!draft?.service_code)return;const token=getSessionToken();if(!token){setMessage("Please sign in before creating a Temp Mail inbox.");return}if(!requestKey.current)requestKey.current=newRequestKey("mail");setBusy(true);setMessage("");try{const result:any=await api.tempMail.order(token,{service_code:draft.service_code,request_key:requestKey.current});const reference=refOf(result);if(!reference)throw new Error("Mail was created but its reference was unavailable.");sessionStorage.setItem(`wickspend.tempMailOrder.${reference}`,JSON.stringify(result?.order||result?.data||result));sessionStorage.removeItem("wickspend.tempMailDraft");router.replace(`/temp-mail/created?reference=${encodeURIComponent(reference)}`)}catch(e){setMessage(e instanceof Error?e.message:"Unable to create Temp Mail");setBusy(false)}}
