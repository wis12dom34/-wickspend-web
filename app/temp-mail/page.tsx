...
const mailExpiry=(m:any)=>String(m?.expires_in||m?.expires_at||m?.ttl||m?.code_expires_in||m?.code_expires_at||"");
...
export default function TempMail(){
  ...
  const status=String(order?.status||"Active"),ended=/cancel|expired|complete|refunded/i.test(status),expired=/expired|complete/i.test(status);
  ...
  if(selectedMail){
    const sender=senderOf(selectedMail),subject=subjectOf(selectedMail),body=bodyOf(selectedMail),mailCode=codeOf(selectedMail)||rootOtp,expiry=mailExpiry(selectedMail);
    return <main className={s.page}>
      <div className={s.backRow}><button className={s.back} onClick={()=>setSelectedMail(null)} aria-label="Back to inbox">‹</button><div className={s.detailTitle}><h1>Verification email</h1><p>{sender} • {receivedAt(selectedMail)}</p></div><span/></div>
      <section className={`${s.card} ${s.detailCard}`}><h2>{subject}</h2><p className={s.to}>To: {email||"Temporary inbox"}</p><p className={s.bodyLead}>{body||"Use this code to finish verifying your account."}</p>{mailCode&&<><div className={s.otpPanel}><small>VERIFICATION CODE</small><strong>{fmtCode(mailCode)}</strong><button className={s.copyCode} onClick={()=>copy(mailCode,"Code")}>Copy code</button></div></>}<p className={s.detailMeta}>{expiry?`Code expiry: ${expiry}`:"Use the code while this temporary inbox is active."}</p><p className={s.detailMuted}>If you did not request it, you can ignore this email.</p><p className={s.signature}>— {sender} account team</p></section>
      <button className={`${s.wideBtn} ${s.wideDark}`} onClick={()=>setMessage(`${sender} link is not available in this message.`)}>Open {sender}</button>
      <button className={`${s.wideBtn} ${s.wideLight}`} disabled title="Message deletion is not exposed by the current Temp Mail API.">Delete email</button>
  ...
  if(order&&ended){
    const terminalLabel=expired?"EXPIRED":"CLOSED",eyebrow=expired?"EMAIL EXPIRED":"EMAIL CLOSED";
    return <main className={s.page}><h1 className={s.title}>Temp Mail</h1><p className={s.subtitle}>Private inbox for verification codes</p><section className={`${s.card} ${s.expiredCard}`}><div className={s.eyebrow}>{eyebrow}</div><div className={s.email}>{email||"Temporary email"}</div><p className={s.expiredText}>This temporary address is no longer active.</p><span className={s.expiredBadge}>{terminalLabel}</span></section><button className={s.newFull} disabled={busy} onClick={newEmail}>{busy?"Preparing…":"Create new temp email"}</button><div className={s.sectionHead}><h2>Inbox</h2></div><div className={`${s.card} ${s.empty}`}><div><div className={s.emptyIcon}>—</div><h3>No messages</h3><p>Create a new email to start receiving verification codes.</p></div></div><div className={s.tip}><b>Temporary by design</b><p>Inactive addresses stop receiving new messages.</p></div>{message&&<p className={s.screenMessage} role="status">{message}</p>}<BottomNav/></main>
  }

  if(order){
    return <main className={`${s.page}${busy?` ${s.loading}`:""}`}><h1 className={s.title}>Temp Mail</h1><p className={s.subtitle}>Private inbox for verification codes</p><section className={`${s.card} ${s.emailCard}`}><div className={s.eyebrow}>YOUR TEMP EMAIL</div><div className={s.email}>{email||"Temporary inbox"}</div><button className={s.copy} onClick={()=>copy(email,"Email")}>Copy</button><div className={s.expiry}>{expires?`Expires in ${expires}`:`Status: ${status}`}{reference?` • ${reference}`:""}</div></section><div className={s.actionRow}><button className={s.action} disabled={busy} onClick={refresh}>Refresh inbox</button><button className={`${s.action} ${s.actionPrimary}`} disabled={busy} onClick={newEmail}>New email</button></div><div className={s.sectionHead}><h2>Inbox</h2><span className={s.badge}>{mails.length}</span></div><div className={s.messages}>{mails.length?mails.map((m:any,i)=><button className={s.message} key={m.id||i} onClick={()=>setSelectedMail(m)}><div className={s.messageTop}>{i===0&&<span className={s.dot}/>}<span>{senderOf(m)}</span><span className={s.time}>{receivedAt(m)}</span></div><strong>{subjectOf(m)}</strong><p>{bodyOf(m)||codeOf(m)||"Open message"}</p></button>):<div className={`${s.card} ${s.empty}`}><div><div className={s.emptyIcon}>—</div><h3>No messages yet</h3><p>Keep this inbox open and tap refresh.</p></div></div>}</div><div className={s.tip}><b>Waiting for a code?</b><p>Keep this inbox open and tap refresh.</p></div>{message&&<p className={s.screenMessage} role="status">{message}</p>}<BottomNav/></main>
  }
