...
export default function ResellerDashboard() {
  ...
  if (mode === "loading") return <main className="resellerShell"><ResellerNav/><div className="resellerLoading">Loading reseller workspace…</div></main>;
...
function Plans({plans,busy,currentPlan,onSubscribe}:{plans:any[];busy:boolean;currentPlan?:string;onSubscribe:(code:string,cycle:"monthly"|"annual")=>void}) {
  return <section className="plansSection"><div className="sectionTitle"><div><span className="eyebrow">Plans</span><h2>Reseller subscription</h2></div></div>{plans.length ? <div className="planGrid">{plans.map((p:any)=><article className="planCard" key={p.code}><div className="cardHead"><div><h3>{p.name}</h3><small>{p.code}</small></div>{currentPlan===p.code&&<span className="status good">Current</span>}</div><div className="planPrices"><div><span>Monthly</span><b>{p.monthly_price_ngn == null ? "Not offered" : money(p.monthly_price_ngn)}</b>{p.monthly_price_ngn != null && <button disabled={busy} onClick={()=>onSubscribe(p.code,"monthly")}>Choose monthly</button>}</div><div><span>Annual</span><b>{p.annual_price_ngn == null ? "Not offered" : money(p.annual_price_ngn)}</b>{p.annual_price_ngn != null && <button disabled={busy} onClick={()=>onSubscribe(p.code,"annual")}>Choose annual</button>}</div></div></article>)}</div> : <div className="emptyState">Reseller plans have not been configured yet.</div>}</section>;
}
