  ...
  return <main className="resellerShell">
    ...
    {loading ? <div className="resellerLoading">Loading subscription billing…</div> : <>
      ...
      <section className="plansSection"><div className="sectionTitle"><div><span className="eyebrow">Available plans</span><h2>Choose a subscription</h2></div></div>
        {plans.length ? <div className="planGrid">{plans.map((p:any)=><article className="planCard" key={p.code}><div className="cardHead"><div><h3>{p.name}</h3><small>{p.code}</small></div>{currentPlan===p.code&&sub?.active&&<span className="status good">Current</span>}</div><div className="featureChips">{p.features?.api_access===true&&<span>API access</span>}{p.features?.custom_domain===true&&<span>Custom domain</span>}{Number(p.features?.api_key_limit||0)>0&&<span>{Number(p.features.api_key_limit)} API key{Number(p.features.api_key_limit)===1?"":"s"}</span>}</div><div className="planPrices"><div><span>Monthly</span><b>{p.monthly_price_ngn==null?"Not offered":money(p.monthly_price_ngn)}</b>{p.monthly_price_ngn!=null&&<button disabled={busy} onClick={()=>subscribe(p.code,"monthly")}>{currentPlan===p.code&&sub?.active?"Renew monthly":"Choose monthly"}</button>}</div><div><span>Annual</span><b>{p.annual_price_ngn==null?"Not offered":money(p.annual_price_ngn)}</b>{p.annual_price_ngn!=null&&<button disabled={busy} onClick={()=>subscribe(p.code,"annual")}>{currentPlan===p.code&&sub?.active?"Renew annual":"Choose annual"}</button>}</div></div></article>)}</div> : <div className="emptyState">No reseller plans are available yet. Your WickSpend administrator must configure and activate a plan first.</div>}
      </section>
