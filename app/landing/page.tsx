...
function NumberPreview({ compact = false }: { compact?: boolean }) {
  return <div className={`glass preview ${compact ? "productPreview" : "previewHero floatPreview"}`}>
...
export default function LandingPage() {
  ...
  return <main className="landingPage">
    <div className="landingWrap">
      <header className={`landingNav fadeIn ${menuOpen ? "menuOpen" : ""}`}>
        <Link href="/landing" className="landingBrand" onClick={closeMenu}>WickSpend</Link>
        <nav className="landingNavLinks" aria-label="Landing navigation">
          <a href="#products">Products</a><a href="#marketplace">Marketplace</a><a href="#wallet">Wallet</a><a href="#faq">Help</a><Link href="/login">Sign in</Link>
        </nav>
        <button className="landingMenu" type="button" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen(v => !v)}>
          <span></span><span></span><span></span>
        </button>
        {menuOpen && <nav className="mobileMenu" aria-label="Mobile landing navigation">
          <a href="#products" onClick={closeMenu}>Products</a><a href="#marketplace" onClick={closeMenu}>Marketplace</a><a href="#wallet" onClick={closeMenu}>Wallet</a><a href="#faq" onClick={closeMenu}>Help</a><Link href="/login" onClick={closeMenu}>Sign in</Link><Link className="mobileMenuPrimary" href="/login" onClick={closeMenu}>Get Started</Link>
        </nav>}
      </header>

      <section className="hero fadeIn"><h1>Digital services, without the friction.</h1><p>Buy virtual numbers, receive OTPs, rent numbers, and access premium digital products from one clean WickSpend account.</p><div className="ctaRow"><Link className="btn btnPrimary" href="/login">Get Started</Link><Link className="btn btnSecondary" href="/marketplace">Explore Marketplace</Link></div></section>
      <NumberPreview />

      <section className="section" id="products"><div className="eyebrow">HOW IT WORKS</div><h2>Three steps. Nothing more.</h2><p className="sectionIntro">WickSpend keeps the buying flow short, clear, and consistent across numbers, OTP, rentals and marketplace products.</p><div className="steps">{[['01','Choose','Pick country + service.'],['02','Purchase','Confirm transparent price.'],['03','Receive','Receive instantly.']].map(([n,t,d]) => <article className="glass step" key={n}><small>{n}</small><h3>{t}</h3><p>{d}</p></article>)}</div></section>

      <section className="section productLayout"><div className="productCopy"><div className="eyebrow">PRODUCT</div><h2>Virtual Numbers</h2><p className="sectionIntro">Choose a country and service, then instantly see an available virtual number with validity and refund rules.</p><Link href="/buy-number" className="textAction">Browse numbers →</Link></div><NumberPreview compact /></section>

      <section className="section productLayout reverse"><div className="productCopy"><div className="eyebrow">PRODUCT</div><h2>Instant OTP</h2><p className="sectionIntro">Stay on one focused screen while WickSpend waits for your verification code and shows the timer, status and refund option.</p><Link href="/orders" className="textAction">View your orders →</Link></div><div className="glass preview productPreview"><div className="previewHeader">WickSpend</div><div className="otpCard"><div><small>Waiting for OTP</small><strong>••••••</strong></div><b>24:57</b></div><div className="numberCard"><div><strong>Verification active</strong><div className="numberMeta">WhatsApp • United States</div></div><Link href="/orders" className="miniBtn">View OTP</Link></div></div></section>

      <section className="section productLayout"><div className="productCopy"><div className="eyebrow">PRODUCT</div><h2>Number Rentals</h2><p className="sectionIntro">Keep a number longer with clear rental periods, pricing and renewal context for repeat OTP use.</p><Link href="/rent-number" className="textAction">Browse rentals →</Link></div><NumberPreview compact /></section>

      <section className="section" id="marketplace"><div className="eyebrow">MARKETPLACE</div><h2>Premium digital products in the same account.</h2><p className="sectionIntro">Browse marketplace categories, search products, check live stock and purchase without leaving WickSpend.</p><div className="glass preview productPreview"><div className="previewHeader">Marketplace</div><Link href="/marketplace" className="selector selectorLink marketplaceSearch"><small>Search products...</small></Link><div className="marketList">{[['9PROXY 10 IP','$4.00 • 126 pcs'],['Premium Account','$7.50 • 18 pcs'],['Residential Proxy','$8.00 • 53 pcs']].map(([name,meta]) => <div className="marketItem" key={name}><div><b>{name}</b><small>{meta}</small></div><Link href="/marketplace" className="miniBtn">Buy</Link></div>)}</div><Link href="/marketplace" className="textAction previewAction">Explore Marketplace →</Link></div></section>
      ...
      <footer className="landingFooter"><div className="brandLine">WickSpend</div><div className="footerLinks"><a href="#products">Products</a><Link href="/buy-number">Numbers</Link><Link href="/marketplace">Marketplace</Link><a href="#wallet">Wallet</a><a href="#faq">Help</a></div><div className="copyright">© 2026 WickSpend. All rights reserved.</div></footer>
    </div>
  </main>;
}
